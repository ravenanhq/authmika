...
const RoleListPage = () => {
  ...
  const addRole = async (id: any, newRole: RowData) => {
    try {
      const response = await RoleApi.addRoleApi(id, newRole);
      setUniqueAlert("");
      if (response) {
  ...
  const getRoleList = async () => {
    try {
      const response = await RoleApi.getAllRoleApi();
      if (response) {
  ...
  const handleDeleteRole = async (selectedRow: any) => {
    if (selectedRow !== null) {
      try {
        ...
        if (itemIndex !== -1) {
          const response = await RoleApi.deleteRoleApi(selectedRow.id);
  ...
  const editRole = async (id: any, updatedData: any) => {
    try {
      const response = await RoleApi.updateRoleApi(id, updatedData);
      setUniqueAlert("");
      if (response) {
        if (response.statusCode === 409) {
          setUniqueAlert(response.message);
